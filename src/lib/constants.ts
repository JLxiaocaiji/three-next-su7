...
/**
 * 安全拷贝 Three.js 材质的 Uniforms（对应 Du）
 */
export function cloneUniforms(uniforms: any): any {
  const result: any = {};

  for (const key in uniforms) {
    result[key] = {};

    for (const prop in uniforms[key]) {
      const value = uniforms[key][prop];
 ...
 * 合并多个 Uniforms（对应 no）
 */
export function mergeUniforms(uniformsArray: any[]): any {
  const merged: any = {};

  for (const uniforms of uniformsArray) {
    const copied = cloneUniforms(uniforms);
...
export const ShaderMaterials = {
  basic: {
    uniforms: mergeUniforms([
      ShaderUniformLib.common,
      ShaderUniformLib.specularmap,
      ShaderUniformLib.envmap,
      ShaderUniformLib.aomap,
      ShaderUniformLib.lightmap,
      ShaderUniformLib.fog,
    ]),
  ...
  lambert: {
    uniforms: mergeUniforms([
      ...
      ShaderUniformLib.aomap,
      ShaderUniformLib.lightmap,
      ShaderUniformLib.emissivemap,
      ShaderUniformLib.bumpmap,
      ShaderUniformLib.normalmap,
      ShaderUniformLib.displacementmap,
      ShaderUniformLib.fog,
      ShaderUniformLib.lights,
  ...
  standard: {
    uniforms: mergeUniforms([
      ...
      ShaderUniformLib.normalmap,
      ShaderUniformLib.displacementmap,
      ShaderUniformLib.roughnessmap,
      ShaderUniformLib.metalnessmap,
      ShaderUniformLib.fog,
      ShaderUniformLib.lights,
      {
        emissive: { value: new THREE.Color(0) },
  ...
  points: {
    uniforms: mergeUniforms([ShaderUniformLib.points, ShaderUniformLib.fog]),
  ...
  depth: {
    uniforms: mergeUniforms([ShaderUniformLib.common, ShaderUniformLib.displacementmap]),
  ...
  sprite: {
    uniforms: mergeUniforms([ShaderUniformLib.sprite, ShaderUniformLib.fog]),
...
// Sa.physical
ShaderMaterials.physical = {
  uniforms: mergeUniforms([
    ShaderMaterials.standard.uniforms,
    {
      clearcoat: { value: 0 },
      clearcoatMap: { value: null },
      clearcoatRoughness: { value: 0 },
      clearcoatRoughnessMap: { value: null },
      clearcoatNormalScale: { value: new THREE.Vector2(1, 1) },
      ...
      sheen: { value: 0 },
      sheenColor: { value: new THREE.Color(0) },
      ...
      transmissionSamplerSize: { value: new THREE.Vector2() },
      ...
      attenuationColor: { value: new THREE.Color(0) },
